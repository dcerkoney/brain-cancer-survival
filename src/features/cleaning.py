...
def clean_age_recode_with_lt1_year_olds(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Age recode with <1 year olds"] = (
        df["Age recode with <1 year olds"].str.slice(0, 2).astype(int)
    )
    return df
...
def convert_integer_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns:
        pd.DataFrame: DataFrame with dtypes changed.
    """
    integer_columns = ["Months from diagnosis to treatment"]
...
def split_X_and_y_data(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    """
    X = df.drop(
        ["Vital status recode (study cutoff used)", "Survival months"], axis="columns"
    )
    y = pd.DataFrame(
        {
            "Event indicator": (
            ...
            "Survival months": df["Survival months"],
        }
    )
    return (X, y)
...
def clean_tumor_size_codes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def drop_tumor_size_codes(df, column, threshold_code):
        ...
        of 999.
        """
        mask = np.logical_and(df[column] > threshold_code, df[column] != 999)
    ...
    df["Combined Tumor Size"] = sizes_88_03.combine_first(sizes_04_15).combine_first(
        sizes_16
    )
